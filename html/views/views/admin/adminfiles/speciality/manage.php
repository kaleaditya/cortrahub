...
<script type="text/javascript">
	...
	function checkspeciality()
	{
		...
		{
			type : "POST",
			url:"<?php echo site_url('admins/speciality/checkspeciality');?>",
	...
	<div id="globalTabContent">
		...
		<div id="tab-1" class="tabs-container">
			<div class="globalContainer">
				...
				<?php echo form_open('admins/speciality/add','name="form1" id="form1" enctype="multipart/form-data" onsubmit="return checkspeciality()"'); ?>
					<table id="roottable" style="display:none;" cellpadding="5" cellspacing="0" border="0" width="100%" summary="" class="tblDisplay widthPc">
						
						<tr class="odd">
							<td class="width130" valign="top">Speciality <span class="highlight">*</span></td>
							<td>
								<input type="text" name="speciality" id="speciality" class="required" onblur="checkspeciality()" style='width:400px;' maxlength="200" minlength="2">
								<span id="errormessage" style="display:none; color:#ff0000">This Speciality already exists
								</span>
							</td>
						</tr>
...
<script type="text/javascript" charset="utf-8">
	...
	function active(id)
	{
		var value= confirm('Are you sure you want to Activate this Speciality?');
		if(value)
		{
			window.location = '<? echo site_url("admins/speciality/active");?>'+"/"+id;
		}
	}

	function inactive(id)
	{
		var value= confirm('Are you sure you want to In activate this Speciality?');
		if(value)
		{
			window.location = '<? echo site_url("admins/speciality/inactive");?>'+"/"+id;
		}
	}

	function renderaction(data, type, full)
	{
		var value = "<a href='<? echo site_url('admins/speciality/update');?>"+'/'+data+"' title='Edit'><img src='<?= site_url('assets/admin/images/editpng.png') ?>'/></a> | <a href='#' onclick='confirm_delete("+data+");' title='Delete'><img src='<?= site_url('assets/admin/images/deletepng.png') ?>'/></a>";
		return value;
	}

	function confirm_delete(id)
	{
		var value= confirm('Are you sure you want to delete this Speciality?');
		if(value)
		{
			window.location = '<? echo site_url("admins/speciality/delete");?>'+"/"+id;
		}
	}
	...
	$(document).ready(function() 
	{
		$('#example').dataTable(
		{
			...
			"sAjaxSource":"<?php echo site_url('admins/speciality/getdata');?>"
			//"sDom": "<''f>t<'F'lp>"
		});
	});
</script>