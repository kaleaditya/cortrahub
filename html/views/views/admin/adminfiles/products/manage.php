...
<script type="text/javascript">
	...
	function checkproduct()
	{
		...
		{
			type : "POST",
			url:"<?php echo site_url('admins/products/checkproduct');?>",
	...
	<div id="globalTabContent">
		...
		<div id="tab-1" class="tabs-container">
			<div class="globalContainer">
				...
				<?php echo form_open('admins/products/add','name="form1" id="form1" enctype="multipart/form-data" onsubmit="return checkproduct()"'); ?>
					<table id="roottable" style="display:none;" cellpadding="5" cellspacing="0" border="0" width="100%" summary="" class="tblDisplay widthPc">
						
						<tr class="odd">
							<td class="width130" valign="top">Product <span class="highlight">*</span></td>
							<td>
								<input type="text" name="product" id="product" class="required" onblur="checkproduct()" style='width:400px;' maxlength="200" minlength="2">
								<span id="errormessage" style="display:none; color:#ff0000">This Product already exists
								</span>
							</td>
						</tr>
...
<script type="text/javascript" charset="utf-8">
	...
	function active(id)
	{
		var value= confirm('Are you sure you want to Activate this Product?');
		if(value)
		{
			window.location = '<? echo site_url("admins/products/active");?>'+"/"+id;
		}
	}

	function inactive(id)
	{
		var value= confirm('Are you sure you want to In activate this Product?');
		if(value)
		{
			window.location = '<? echo site_url("admins/products/inactive");?>'+"/"+id;
		}
	}

	function renderaction(data, type, full)
	{
		var value = "<a href='<? echo site_url('admins/products/update');?>"+'/'+data+"' title='Edit'><img src='<?= site_url('assets/admin/images/editpng.png') ?>'/></a> | <a href='#' onclick='confirm_delete("+data+");' title='Delete'><img src='<?= site_url('assets/admin/images/deletepng.png') ?>'/></a>";
		return value;
	}
	...
	function confirm_delete(id)
	{
		...
		if(value)
		{
			window.location = '<? echo site_url("admins/products/delete");?>'+"/"+id;
		}
	}
	...
	$(document).ready(function() 
	{
		$('#example').dataTable(
		{
			...
			"sAjaxSource":"<?php echo site_url('admins/products/getdata');?>"
			//"sDom": "<''f>t<'F'lp>"
		});
	});
</script>