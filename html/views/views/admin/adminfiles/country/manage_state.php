...
<script type="text/javascript">
	...
	function checkstate()
	{
		...
		{
			type : "POST",
			url:"<?php echo site_url('admins/country/checkstate');?>",
...
<div id="main">
	<div id="globalTabContent">
		<ul class="tabs-nav">
			...
			<li><a href="<?php echo site_url('admins/country');?>" id="back" class="button-big">Back to List View</a></li>
        </ul>
		<div id="tab-1" class="tabs-container">
			<div class="globalContainer">
				...
				<?php echo form_open('admins/country/add_state','name="form1" id="form1" enctype="multipart/form-data" onsubmit="return checkstate()"'); ?>
					<table id="roottable" style="display:none;" cellpadding="5" cellspacing="0" border="0" width="100%" summary="" class="tblDisplay widthPc">
						
						<tr class="odd">
							<td class="width130" valign="top">State <span class="highlight">*</span></td>
							<td>
								<input type="text" name="state" id="state" class="required" onblur="checkstate()" style='width:400px;' maxlength="200" minlength="2">
...
<script type="text/javascript" charset="utf-8">
	...
	{
		var countryid = '<?=$countryid;?>';
		var value= confirm('Are you sure you want to Activate this State?');
		if(value)
		{
			window.location = '<? echo site_url("admins/country/active_state");?>'+"/"+id+"/"+countryid;
		}
	}
	...
	{
		var countryid = '<?=$countryid;?>';
		var value= confirm('Are you sure you want to In activate this State?');
		if(value)
		{
			window.location = '<? echo site_url("admins/country/inactive_state");?>'+"/"+id+"/"+countryid;
		}
	}
	...
	{
		var countryid = '<?=$countryid;?>';
		var value = "<a href='<?=site_url('admins/country/manage_city')?>/"+data+"/"+countryid+"'>Manage city</a>";
		return value;
	}
	...
	{
		var countryid = '<?=$countryid;?>';
		var value = "<a href='<? echo site_url('admins/country/update_state');?>"+'/'+data+"/"+countryid+"' title='Edit'><img src='<?= site_url('assets/admin/images/editpng.png') ?>'/></a> | <a href='#' onclick='confirm_delete("+data+");' title='Delete'><img src='<?= site_url('assets/admin/images/deletepng.png') ?>'/></a>";
		return value;
	}
	...
	{
		var countryid = '<?=$countryid;?>';
		var value= confirm('Are you sure you want to delete this State?');
		if(value)
		{
			window.location = '<? echo site_url("admins/country/delete_state");?>'+"/"+id+"/"+countryid;
		}
	}
	...
	$(document).ready(function() 
	{
		$('#example').dataTable(
		{
			...
			"sAjaxSource":"<?php echo site_url('admins/country/get_state');?>/<?=$countryid;?>"
			//"sDom": "<''f>t<'F'lp>"
		});
	});
</script>