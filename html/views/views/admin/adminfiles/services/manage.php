...
<script type="text/javascript">
	...
	function checkservice()
	{
		...
		{
			type : "POST",
			url:"<?php echo site_url('admins/services/checkservice');?>",
	...
	<div id="globalTabContent">
		...
		<div id="tab-1" class="tabs-container">
			<div class="globalContainer">
				...
				<?php echo form_open('admins/services/add','name="form1" id="form1" enctype="multipart/form-data" onsubmit="return checkservice()"'); ?>
					<table id="roottable" style="display:none;" cellpadding="5" cellspacing="0" border="0" width="100%" summary="" class="tblDisplay widthPc">
						...
						<tr class="odd">
							...
							<td>
								<input type="text" name="service" id="service" class="required" onblur="checkservice()" style='width:400px;' maxlength="200" minlength="2">
...
<script type="text/javascript" charset="utf-8">
	...
	function active(id)
	{
		var value= confirm('Are you sure you want to Activate this Services?');
		if(value)
		{
			window.location = '<? echo site_url("admins/services/active");?>'+"/"+id;
		}
	}

	function inactive(id)
	{
		var value= confirm('Are you sure you want to In activate this Services?');
		if(value)
		{
			window.location = '<? echo site_url("admins/services/inactive");?>'+"/"+id;
		}
	}

	function renderaction(data, type, full)
	{
		var value = "<a href='<? echo site_url('admins/services/update');?>"+'/'+data+"' title='Edit'><img src='<?= site_url('assets/admin/images/editpng.png') ?>'/></a> | <a href='#' onclick='confirm_delete("+data+");' title='Delete'><img src='<?= site_url('assets/admin/images/deletepng.png') ?>'/></a>";
		return value;
	}

	function confirm_delete(id)
	{
		var value= confirm('Are you sure you want to delete this Service?');
		if(value)
		{
			window.location = '<? echo site_url("admins/services/delete");?>'+"/"+id;
		}
	}
	...
	$(document).ready(function() 
	{
		$('#example').dataTable(
		{
			...
			"sAjaxSource":"<?php echo site_url('admins/services/getdata');?>"
			//"sDom": "<''f>t<'F'lp>"
		});
	});
</script>