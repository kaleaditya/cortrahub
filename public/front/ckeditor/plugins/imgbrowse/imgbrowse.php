...
class imgbrowse {
  // IN $root - REPLACE "root/imgup" WITH THE PATH TO THE FOLDER WITH IMAGES RELATIVE TO ROOT OF YOUR WEBSITE ON SERVER
  protected $root = 'userfiles';
  protected $imgext = array('bmp', 'gif', 'jpg', 'jpe', 'jpeg', 'png');// allowed image extensions
  ...
  public function getMenuImgs() {
    ...
    $protocol = $_SERVER['HTTPS']=='off' ? 'http://' : 'http://';
    $site = $protocol. $_SERVER['SERVER_NAME'] .'/';

    // traverse the $obdr
    foreach($obdr as $fileobj) {
      $name = $fileobj->getFilename();
 	  $ext = strtolower(pathinfo($fileobj->getFilename(), PATHINFO_EXTENSION));
      // if image file, else, directory (but not . or ..), add data in $re

      if($fileobj->isFile() && in_array($ext, $this->imgext)) $re['imgs'] .= '<span><img src="'. $site . $this->root . $this->imgdr . $name .'" alt="'. $name .'" height="50" />'. $name .'</span>';
