  ...
  <section class="py-5 text-center">
    <div class="container">
      ...
      <div class="row row-cols-1 row-cols-md-4 g-4 mt-4">
        ...
        <div class="col wow animate__animated animate__fadeInUp" data-wow-delay="0.2s">
          <div class="card h-100">
           <img src="{{asset('public/all_image/'.$user->image)}}" class="card-img-top" alt="{{$user->name}}">
            <div class="card-body text-start">
              <h5 class="card-title">{{$user->name}}</h5>
              <p class="card-text">{{$user->qualification}}</p>
              <p class="card-text">{{$user->city_info->name}}</p> 
            </div>
          </div>
        </div>
        @endforeach
